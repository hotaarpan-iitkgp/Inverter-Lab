...
export interface PowerSimAppProps {
...
export default function PowerSimApp({ activeAppId = 'powersim', onSelectApp }: PowerSimAppProps) {
  ...
  return (
    <div className="flex flex-col w-full h-screen bg-[#090B0E] relative overflow-hidden">
      {/* Loading Overlay */}
      {isLoading && (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#090B0E] z-20 gap-3 px-4 text-center">
          ...
          <div className="flex items-center gap-2 mt-2">
            <button
              ...
              className="px-3 py-1.5 rounded bg-slate-800 hover:bg-slate-700 text-xs font-medium text-white border border-slate-700 cursor-pointer"
            >
              Try Again
            </button>
            <a
      ...
      {/* Embedded Simulation Canvas Frame with dynamically resolved URL (Full Screen Height, No Extra Header) */}
      <iframe
        key={iframeKey}
        src={powerSimUrl}
