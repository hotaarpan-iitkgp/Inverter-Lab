      ...
      <g id="circuit-three-phase-complete" className="animate-in fade-in duration-200">
        ...
        {topology.includes('3L') ? (
          <g id="tp-dc-caps">
            ...
            {topology !== '3L-TNPC' ? (
              <path
                d="M 140 240 L 274 240 A 6 6 0 0 1 286 240 L 454 240 A 6 6 0 0 1 466 240 L 592 240"
                fill="none"
                stroke="#475569"
                strokeWidth="1.5"
                strokeDasharray="3,3"
            ...
            <text x="110" y={topology === '3L-TNPC' ? "184" : "244"} textAnchor="middle" fill="#eab308" className="text-[8px] font-mono font-bold">NP (0V)</text>
          </g>
        ) : (
        ...
        {topology === '3L-TNPC' && (
          <g id="tp-legs-3l-tnpc">
            {/* Dedicated Neutral Point Bus at y = 180 distributing from DC midpoint (140, 240) */}
            <g id="tp-tnpc-np-bus">
              {/* Feeder vertical line from capacitor midpoint (140, 240) up to NP bus (140, 180) */}
              <line
                x1="140"
                y1="240"
                x2="140"
                y2="180"
                stroke={stateA === 'O' || stateB === 'O' || stateC === 'O' ? '#10b981' : '#f59e0b'}
                strokeWidth={stateA === 'O' || stateB === 'O' || stateC === 'O' ? '2.5' : '1.5'}
                className={stateA === 'O' || stateB === 'O' || stateC === 'O' ? 'active-current-path' : ''}
              />
              <circle cx="140" cy="180" r="3.5" fill="#f59e0b" />

              {/* Segment 1: from DC link (140, 180) to Phase A tap (180, 180) */}
              <line
                x1="140"
                y1="180"
                x2="180"
              ...
              {/* Segment 2: from Phase A tap (180, 180) across Leg A (with jumper arc) to Phase B tap (360, 180) */}
              <path
                d="M 180 180 L 274 180 A 6 6 0 0 1 286 180 L 360 180"
                fill="none"
                stroke={stateB === 'O' || stateC === 'O' ? '#10b981' : '#f59e0b'}
                strokeWidth={stateB === 'O' || stateC === 'O' ? '2.5' : '1.5'}
                strokeDasharray="4 2"
                className={stateB === 'O' || stateC === 'O' ? 'active-current-path' : ''}
              />

              {/* Segment 3: from Phase B tap (360, 180) across Leg B (with jumper arc) to Phase C tap (540, 180) */}
              <path
                d="M 360 180 L 454 180 A 6 6 0 0 1 466 180 L 540 180"
                fill="none"
                stroke={stateC === 'O' ? '#10b981' : '#f59e0b'}
                strokeWidth={stateC === 'O' ? '2.5' : '1.5'}
                strokeDasharray="4 2"
                className={stateC === 'O' ? 'active-current-path' : ''}
              />

              {/* Phase Neutral Point Tap Junction Dots */}
              <circle cx="180" cy="180" r="3.5" fill="#f59e0b" />
              <circle cx="360" cy="180" r="3.5" fill="#f59e0b" />
              <circle cx="540" cy="180" r="3.5" fill="#f59e0b" />
            </g>

            {/* Phase A: Leg A at x = 280 */}
            <g id="tp-tnpc-leg-a">
              <line x1="280" y1="60" x2="280" y2="420" stroke="#334155" strokeWidth="1.5" />
              {renderSwitch(280, 130, 'S_A1', stateA === 'P', 'right')}
              {renderSwitch(280, 350, 'S_A4', stateA === 'N', 'right')}

              {/* T-branch path from NP tap at (180, 180) down to y=195 and into Leg A at (280, 195) */}
              <path
                d="M 180 180 L 180 195 L 280 195"
                fill="none"
                stroke={stateA === 'O' ? '#10b981' : '#475569'}
                strokeWidth={stateA === 'O' ? '2.5' : '1.5'}
                className={stateA === 'O' ? 'active-current-path' : ''}
              />
              {renderSwitch(205, 195, 'S_A2', stateA === 'O', 'top', { orientation: 'horizontal' })}
              {renderSwitch(245, 195, 'S_A3', stateA === 'O', 'bottom', { orientation: 'horizontal', flipHorizontal: true })}

              {/* Junction dot on Leg A where T-branch connects */}
              <circle cx="280" cy="195" r="3.5" fill={stateA === 'O' ? '#10b981' : '#f59e0b'} />
              {/* Motor output node at y=215 */}
              <circle cx="280" cy="215" r="4" fill="#38bdf8" />
            </g>

            {/* Phase B: Leg B at x = 460 */}
            <g id="tp-tnpc-leg-b">
              <line x1="460" y1="60" x2="460" y2="420" stroke="#334155" strokeWidth="1.5" />
              {renderSwitch(460, 130, 'S_B1', stateB === 'P', 'right')}
              {renderSwitch(460, 350, 'S_B4', stateB === 'N', 'right')}

              {/* T-branch path from NP tap at (360, 180) down to y=195 and into Leg B at (460, 195) */}
              <path
                d="M 360 180 L 360 195 L 460 195"
                fill="none"
                stroke={stateB === 'O' ? '#10b981' : '#475569'}
                strokeWidth={stateB === 'O' ? '2.5' : '1.5'}
                className={stateB === 'O' ? 'active-current-path' : ''}
              />
              ...
              {/* Junction dot on Leg B where T-branch connects */}
              <circle cx="460" cy="195" r="3.5" fill={stateB === 'O' ? '#10b981' : '#f59e0b'} />
              {/* Motor output node at y=240 */}
              <circle cx="460" cy="240" r="4" fill="#38bdf8" />
            </g>

            {/* Phase C: Leg C at x = 640 */}
            <g id="tp-tnpc-leg-c">
              <line x1="640" y1="60" x2="640" y2="420" stroke="#334155" strokeWidth="1.5" />
              {renderSwitch(640, 130, 'S_C1', stateC === 'P', 'right')}
              {renderSwitch(640, 350, 'S_C4', stateC === 'N', 'right')}

              {/* T-branch path from NP tap at (540, 180) down to y=195 and into Leg C at (640, 195) */}
              <path
                d="M 540 180 L 540 195 L 640 195"
                fill="none"
                stroke={stateC === 'O' ? '#10b981' : '#475569'}
                strokeWidth={stateC === 'O' ? '2.5' : '1.5'}
                className={stateC === 'O' ? 'active-current-path' : ''}
              />
              {renderSwitch(565, 195, 'S_C2', stateC === 'O', 'top', { orientation: 'horizontal' })}
              {renderSwitch(605, 195, 'S_C3', stateC === 'O', 'bottom', { orientation: 'horizontal', flipHorizontal: true })}

              {/* Junction dot on Leg C where T-branch connects */}
              <circle cx="640" cy="195" r="3.5" fill={stateC === 'O' ? '#10b981' : '#f59e0b'} />
